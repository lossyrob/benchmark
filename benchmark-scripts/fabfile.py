...
from fabric.api import put, run, shell_env, env
import boto
# for fabric
env.user = 'ubuntu'
env.host_string = 'ec2-52-7-171-110.compute-1.amazonaws.com'  # The public-facing DNS we want fabric to point at
env.key_filename = '~/.ssh/geotrellis-spark-us-east-1.pem'
...
# Send to remote
benchmark_jar = os.path.join('../spark/target/scala-2.10/benchmark-spark-assembly-0.1.0.jar')
...
def benchmark(memory, aws_profile='default', local_dirs='/media/ephemeral0'):
    aws_id, aws_key = get_credentials(aws_profile)
    environment = {
        'JAR': '/tmp/benchmark.jar',
        'AWS_ID': aws_id,
